...
public class ModificaEsercizioListener {
	...
	public void registraEsercizio(ActionEvent e)
	{
		if(verifyField()) //se tutti i campi sono corretti
		{
			//invio al server
			c.sendMsg("!!^^");
			c.sendMsg(Integer.toString(getIdEsercizio(e)));
			...
			if(c.readResponse().equals("-1"))
				...
				message(AlertType.INFORMATION, "MODIFICA AVVENUTA CON SUCCESSO", " ", "");
			
		}

	}
	
	public void registraEsercizioDelete(ActionEvent e)
	{
		registraEsercizio(e);
		c.sendMsg("$$$!"); //elimina carichi
		c.sendMsg(Integer.toString(getIdEsercizio(e)));
	}
